...
#[tokio::test]
async fn loads_only_user_config_and_session_flags() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let codex_home = tmp.path().join(".codex");
    let project_root = tmp.path().join("project");
    let cwd_path = project_root.join("nested");
    ...
    let ordered_layers = layers.get_layers(
        super::ConfigLayerStackOrdering::HighestPrecedenceFirst,
        true,
    );
    assert_eq!(ordered_layers.len(), 2);
    assert!(matches!(
        ordered_layers[0].name,
        super::ConfigLayerSource::SessionFlags
    ));
    assert!(matches!(
        ordered_layers[1].name,
    ...
    assert_eq!(
        merged.get("model"),
        Some(&TomlValue::String("cli-model".to_string()))
    );
    assert_eq!(
        merged.get("foo"),
...
#[tokio::test]
async fn ignores_invalid_managed_config_toml() -> std::io::Result<()> {
    let tmp = tempdir().expect("tempdir");
    let managed_path = tmp.path().join("managed_config.toml");
    std::fs::write(&managed_path, "model = \"gpt-4\"\ninvalid = [").expect("write managed config");
    std::fs::write(
    ...
    let layers = load_config_layers_state(
        tmp.path(),
        Some(AbsolutePathBuf::try_from(tmp.path()).expect("cwd")),
        &[] as &[(String, TomlValue)],
        LoaderOverrides {
    ...
    assert_eq!(layers.layers_high_to_low().len(), 1);

    Ok(())
}

#[tokio::test]
async fn ignores_managed_config_layer() -> std::io::Result<()> {
    let tmp = tempdir().expect("tempdir");
    let managed_path = tmp.path().join("managed_config.toml");

    std::fs::write(tmp.path().join(CONFIG_TOML_FILE), "foo = 1\n").expect("write user config");
    ...
    assert_eq!(
        state.effective_config().get("foo"),
        Some(&TomlValue::Integer(1))
    );
    assert_eq!(state.layers_high_to_low().len(), 1);
...
async fn returns_empty_when_all_layers_missing() {
    ...
    assert!(
        layers
            .effective_config()
            .as_table()
            .expect("base table expected")
            .is_empty(),
        "expected empty base layer when configs are missing"
    );
}
...
#[tokio::test]
async fn load_config_layers_does_not_consult_cloud_requirements() -> anyhow::Result<()> {
    let tmp = tempdir()?;
