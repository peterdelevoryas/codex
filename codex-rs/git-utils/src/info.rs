...
pub fn scrub_git_remote_url(url: &str) -> String {
    ...
    let Some(scheme_end) = without_query_or_fragment.find("://") else {
        let Some(userinfo_end) = without_query_or_fragment.find('@') else {
            return without_query_or_fragment.to_string();
        };
        let userinfo = &without_query_or_fragment[..userinfo_end];
        if !userinfo.contains(':') {
            return without_query_or_fragment.to_string();
        }

        return without_query_or_fragment[userinfo_end + 1..].to_string();
    };
...
mod tests {
    ...
    #[test]
    fn scrub_git_remote_url_preserves_standard_scp_like_git_urls() {
        assert_eq!(
